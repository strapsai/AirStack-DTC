"""Single-drone Agile Flight ground commander — VELOCITY-command policy.

Agile-Flight (Loquercio / agile_autonomy, ckpt-50) analog of
``diffaero_velocity_commander.py``: same ``std_srvs/Trigger`` lifecycle, same
world-ENU velocity setpoint to PX4 (the flight controller owns attitude), same
FlightState machine + geofence + markers. The one substantive difference is the
policy core: the Loquercio net outputs a *position* trajectory, so a velocity
setpoint is recovered by tracking it with the acados RPG-MPC and reading the
velocity straight out of the solved MPC state — see ``agile/agile_core.py``.

Perception is a metric **depth image** (sensor_msgs/Image), not the DiffAero
9×16 ToF grid: the net consumes a 224×224 depth frame directly.

Interfaces (topic templates come from the config YAML):

    state in:    {state_topic_template}               nav_msgs/Odometry (ENU)
    depth in:    {depth_topic_template}                sensor_msgs/Image (32FC1/16UC1)
...
Yaw: the policy noses into its direction of travel. The commander runs a
P-controller from the current heading toward the policy's desired heading and
...
from svg_ground_control.agile.agile_core import AgileObs, AgileVelPolicy
...
class DroneHandle:
    ...
    def __init__(self, name: str, mode: str):
        ...
        self.depth: np.ndarray | None = None      # latest metric planar depth (H,W) meters
        self.last_depth_time = None


class AgileVelocityCommander(Node):

    def __init__(self, **kwargs):
        super().__init__('agile_velocity_commander', **kwargs)
        ...
        # Perception: metric depth image the Loquercio net consumes directly.
        self.declare_parameter('depth_topic_template',
                               '/{name}/sensors/front_stereo/left/depth')
        self.declare_parameter('depth_reliability', 'best_effort')  # reliable | best_effort
        self.declare_parameter('depth_image_topic_template',
                               '/svg/{name}/agile_depth_image')  # debug colormap
        self.declare_parameter('depth_viz_max_dist_m', 10.0)

        self.declare_parameter('checkpoint_path',
                               '/airlab-storage/chiron/models/loquercio/ckpt-50')
        # Loquercio net / MPC knobs.
        self.declare_parameter('backend', 'tensorflow')  # tensorflow | tflite
        self.declare_parameter('tflite_path',
                               '/airlab-storage/chiron/models/loquercio/ckpt-50_float32.tflite')
        ...
        self.declare_parameter('cruise_alt_m', 1.2)      # MPC hold altitude when alt_hold
        self.declare_parameter('mpc_alt_hold', True)     # hold cruise_alt (True) vs follow net z
        self.declare_parameter('depth_max_m', 20.0)
        # target_vel saturation toward goal + MPC speed cap.
        ...
        depth_tmpl = str(self.get_parameter('depth_topic_template').value)
        depth_image_tmpl = str(self.get_parameter('depth_image_topic_template').value)
        self.depth_reliability = str(self.get_parameter('depth_reliability').value)
        self.depth_viz_max_dist = float(self.get_parameter('depth_viz_max_dist_m').value)
        ...
        self.create_subscription(
            Image, depth_tmpl.format(name=name),
            self.depth_callback, self._depth_qos(self.depth_reliability))
        ...
        # ---- Agile Flight (Loquercio) velocity policy -------------------
        checkpoint_path = str(self.get_parameter('checkpoint_path').value)
        self.policy: AgileVelPolicy | None = None
        ...
        if checkpoint_path:
            self.policy = AgileVelPolicy(
                checkpoint_path=checkpoint_path,
                backend=str(self.get_parameter('backend').value),
                tflite_path=str(self.get_parameter('tflite_path').value),
                ...
                cruise_alt=float(self.get_parameter('cruise_alt_m').value),
                max_vel=float(self.get_parameter('max_vel').value),
                alt_hold=bool(self.get_parameter('mpc_alt_hold').value),
                lookahead_step=int(self.get_parameter('lookahead_step').value),
                depth_max_m=float(self.get_parameter('depth_max_m').value),
            )
            self.get_logger().info(
                f'Agile/Loquercio velocity policy loaded ({self.policy.model_label})')
            # Warm up the net + acados MPC now, in __init__ — the first compute()
            # otherwise costs the net's CUDA init + the MPC's SQP warm start and
            # would stall the control loop at policy handoff.
            try:
                warm = AgileObs(
                    position_enu=np.zeros(3), velocity_enu=np.zeros(3),
                    R_enu=np.eye(3), goal_enu=np.array([1.0, 0.0, 0.0]),
                    depth_planar=None)
                for _ in range(3):
                    self.policy.compute(warm)
                self.policy.reset()
                self.get_logger().info('Agile/Loquercio velocity policy warmed up')
        ...
        # Depth debug image — the metric depth the net consumes, republished as a
        # colormapped Image (red=near obstacle, blue=far) for an RViz/Foxglove
        # Image panel.
        self.depth_image_pub = (
            self.create_publisher(Image, depth_image_tmpl.format(name=name), 10)
            if self.publish_viz else None)
        ...
        self.get_logger().info(
            f'AgileVelocityCommander up | {name} | mode={mode} | scenario={scenario_name}'
    ...
    def odometry_callback(self, msg: Odometry):
        ...
        # The policy expects world-ENU velocity. REP-145 says the twist
    ...
    @staticmethod
    def _depth_qos(reliability: str) -> QoSProfile:
        r = str(reliability).strip().lower()
        policy = (ReliabilityPolicy.BEST_EFFORT
                  if r in ('best_effort', 'besteffort', 'best-effort')
                  else ReliabilityPolicy.RELIABLE)
        return QoSProfile(depth=5, reliability=policy)

    def depth_callback(self, msg: Image):
        """Decode a sensor_msgs/Image depth frame to metric planar depth (meters)."""
        planar = self._decode_depth(msg)
        if planar is None:
            return
        self.drone.depth = planar
        self.drone.last_depth_time = self.get_clock().now()
        self._publish_depth_image(planar)

    def _decode_depth(self, msg: Image) -> np.ndarray | None:
        if msg.is_bigendian:
            self.get_logger().warn('Big-endian depth images are not supported.',
                                   throttle_duration_sec=1.0)
            return None
        encoding = msg.encoding.upper()
        if encoding in ('32FC1', '32FC'):
            dtype, item_size, to_m = np.float32, 4, 1.0        # meters
        elif encoding in ('16UC1', 'MONO16'):
            dtype, item_size, to_m = np.uint16, 2, 1e-3        # mm -> m
        else:
        ...
        try:
            depth = data.reshape(
                (int(msg.height), elems_per_row))[:, :int(msg.width)].astype(np.float32)
        except ValueError:
            self.get_logger().warn('Depth image buffer shape mismatch.',
                                   throttle_duration_sec=1.0)
            return None
        return depth * to_m

    def _publish_depth_image(self, planar_m: np.ndarray):
        """Colormap the metric depth (red=near, blue=far) for an Image panel."""
        if self.depth_image_pub is None:
            return
        z = np.asarray(planar_m, dtype=np.float32)
        valid = np.isfinite(z) & (z > 1e-3)
        closeness = np.zeros_like(z, dtype=np.float32)
        md = max(0.1, self.depth_viz_max_dist)
        closeness[valid] = 1.0 - np.clip(z[valid], 0.0, md) / md
        rgb = np.zeros((*z.shape, 3), dtype=np.uint8)
        rgb[valid, 0] = (closeness[valid] * 255.0).astype(np.uint8)         # red = near
        rgb[valid, 2] = ((1.0 - closeness[valid]) * 255.0).astype(np.uint8)  # blue = far
        h, w = z.shape
        img = Image()
        ...
        self.depth_image_pub.publish(img)
    ...
    def control_loop(self):
        ...
        if d.state == FlightState.IDLE:
            ...
            if reached_goal:
                # The agile policy is a cruise controller, not a position-hold
                ...
                depth_fresh = (
                    d.last_depth_time is not None
                    and (now - d.last_depth_time) < Duration(seconds=self.depth_timeout)
                )
                obs = AgileObs(
                    ...
                    depth_planar=d.depth if depth_fresh else None,
                )
                ...
                self.get_logger().info(
                    ...
                    f'goal={np.round(self.policy_goal,2)} depth_fresh={depth_fresh}',
    ...
    def _world_to_body(velocity_enu: np.ndarray, q: np.ndarray) -> np.ndarray:
        ...
        yaw = AgileVelocityCommander._yaw_from_quat(q)
