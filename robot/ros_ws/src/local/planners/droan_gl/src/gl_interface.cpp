...
/**
 * @brief Constructor for GLInterface
 * @param node Pointer to the ROS node for parameter access and logging
 * @param tf_buffer Pointer to TF buffer for coordinate transformations
 * 
 * Initializes GL interface with ROS parameters and prepares for
 * GPU-based disparity expansion and trajectory collision checking.
 */
    ...
    : node(node), tf_buffer(tf_buffer)
{
  ...
  look_ahead_frame = airstack::get_param(node, "look_ahead_frame", std::string("look_ahead_point_stabilized"));
...
/**
 * @brief Process camera intrinsics and initialize OpenGL context
 * @param msg Camera info message with intrinsic parameters
 * 
 * Extracts camera intrinsics (focal length, principal point, baseline)
 * and initializes the OpenGL context for GPU processing. Only processes
...
/**
 * @brief Process disparity image and perform GPU-based obstacle expansion
 * @param msg Disparity image message from stereo camera
 * 
 * Performs the following GPU operations:
 * 1. Updates graph node with current camera pose
 * 2. Uploads disparity image to GPU texture
 * 3. Executes horizontal and vertical expansion passes
 * 4. Generates foreground (close obstacles) and background (far obstacles) layers
 * 
 * Graph nodes are added based on distance/angle thresholds to maintain
 * recent obstacle history for robust collision checking.
 */
void GLInterface::handle_disparity(const stereo_msgs::msg::DisparityImage::SharedPtr msg)
{
  ...
  if ((graph.size() < 2) ||
      ((graph_node.tf.getOrigin().distance(graph[1].tf.getOrigin()) > graph_distance_threshold) || (tf2::angleShortestPath(graph_node.tf.getRotation(), graph[1].tf.getRotation()) > graph_angle_threshold)))
  {
...
/**
 * @brief Publish visualization of expanded disparity and graph nodes
 * @param hdr Header for timestamp and frame information
 * @param fg_pub Publisher for foreground expanded image
 * @param bg_pub Publisher for background expanded image
 * @param fg_bg_cloud_pub Publisher for 3D point cloud visualization
 * @param marker_pub Publisher for camera pose markers
 * 
 * Generates visualizations of:
 * - Expanded foreground/background disparity images
 * - 3D point cloud of expanded obstacles from all graph nodes
 * - Camera pose arrows showing graph node locations
 */
...
/**
 * @brief Generate and evaluate trajectories using GPU
 * @param look_ahead Odometry of the look-ahead planning point
 * @param trajectory_points Output vector of trajectory points with collision info
 * @param look_ahead_to_target_tf Output transform from look-ahead frame to target frame
 * 
 * Executes GPU-based trajectory generation and collision checking:
 * 1. Transforms look-ahead position to target frame
 * 2. Generates trajectory library using snap dynamics (GPU compute shader)
 * 3. Checks all trajectories against all graph nodes for collisions (GPU)
 * 4. Returns trajectory points with collision status for each graph node
 * 
...
void GLInterface::evaluate_trajectories(const airstack_msgs::msg::Odometry &look_ahead, std::vector<TrajectoryPoint> &trajectory_points, tf2::Transform &look_ahead_to_target_tf)
{
  ...
  *ptr = State(look_ahead_odom);
  ...
  TrajectoryPoint *output_data = (TrajectoryPoint *)glMapBuffer(GL_SHADER_STORAGE_BUFFER, GL_READ_ONLY);
...
/**
 * @brief Get the number of trajectory points per trajectory
 * @return Number of discrete points along each trajectory
 * 
 * Calculated as horizon time divided by time step.
 */
...
/**
 * @brief Check for OpenGL errors and log to console
 * 
 * Utility function for debugging OpenGL operations.
 */
void GLInterface::check_gl_error()
{
  GLenum e = glGetError();
  if (e != GL_NO_ERROR)
    RCLCPP_INFO(node->get_logger(), "GL error: 0x%x", e);
  else
    RCLCPP_INFO(node->get_logger(), "GL error: ok");
}

/**
 * @brief Start GPU timing query
 * 
 * Begins timing of subsequent GPU operations using OpenGL queries.
 */
...
/**
 * @brief End GPU timing query and return elapsed time
 * @return Elapsed time in milliseconds
 * 
 * Completes timing query and returns GPU execution time.
 */
...
/**
 * @brief Initialize OpenGL context and GPU resources
 * @param original_width Original width of input disparity images
 * @param original_height Original height of input disparity images
 * @param downsampled_width Downsampled width for processing
 * @param downsampled_height Downsampled height for processing
 * 
 * Initializes:
 * - OpenGL context (EGL or GLFW)
 * - Trajectory library parameters (pitch/yaw combinations)
 * - Compute shaders for disparity expansion, trajectory generation, and collision checking
 * - GPU textures and buffers for disparity data and trajectories
 * - Uniform buffers for camera intrinsics and trajectory parameters
 */
void GLInterface::initGL(int original_width, int original_height, int downsampled_width, int downsampled_height)
{
  ...
  glUniform1f(glGetUniformLocation(traj_shader, "seen_radius"), seen_radius);
  glUseProgram(0);
  ...
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, traj_ssbo);
  glBufferData(GL_SHADER_STORAGE_BUFFER, traj_params.size() * get_traj_size() * sizeof(TrajectoryPoint), nullptr, GL_DYNAMIC_COPY);
...
/**
 * @brief Compile and link an OpenGL compute shader from file
 * @param file Path to compute shader source file
 * @return OpenGL program handle
 * 
 * Loads, compiles, and links a compute shader program. Logs any
